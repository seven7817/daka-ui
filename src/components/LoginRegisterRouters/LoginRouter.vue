  ...
  <div class="login-content">
    <div class="account-wra" >
        ...
        <div class="account-inp">
            <input type="text" placeholder="请输入邮箱" v-model="Email" @keyup="showDelete1()" @blur.prevent="checkEmail()">
        </div>
    ...
    <div class="password-wra">
        ...
        <div class="password-inp">
            <input type="password" placeholder="请输入密码(6-18位)" v-model="password" @keyup="checkPassword(),showDelete2()" >
        </div>
        <div class="password-delete" v-if="show2" @click="deletePassword()" ></div>
    </div>
    <div class="tips2">{{tips2}}</div>
    <div class="submit-wra">登 录</div>
  </div>
</template>
<script>
export default {
